import {Engine, Scene, ArcRotateCamera, Light, Vector3, HemisphericLight} from 'babylonjs';
...
export class Game {
  ...
  createScene(): void {
      ...
      BABYLON.SceneLoader.Append('../../../assets/scenes/', 'Chungus.stl', this._scene, function (scene) {

        // Create a default skybox with an environment.
        let skybox = BABYLON.MeshBuilder.CreateBox('skyBox', {size:500.0}, scene);
        let skyboxMaterial = new BABYLON.StandardMaterial('skyBox', scene);
        skyboxMaterial.backFaceCulling = false;
        skyboxMaterial.diffuseColor = new BABYLON.Color3(0, 0, 0);
        skyboxMaterial.specularColor = new BABYLON.Color3(0, 0, 0);
        skyboxMaterial.reflectionTexture = new BABYLON.CubeTexture('../../../assets/textures/sea', scene);
   ...
   const mat = new BABYLON.StandardMaterial('mat1', this._scene);
   mat.alpha = .85;
   ...
   const TruncatedIcosidodecahedron = {
     ...
     'face':[[0,1,4,2],[3,8,14,7],[5,10,18,11],[6,12,20,13],[9,17,25,16],[15,24,35,23],[19,28,41,29],[21,31,44,32],[22,33,45,34],[26,38,50,37],[27,40,51,39],[30,43,54,42],[36,48,60,49],[46,55,67,58],[47,59,65,53],[52,64,73,61],[56,62,74,68],[57,70,81,69],[63,75,87,76],[66,78,90,79],[71,82,94,83],[72,85,95,84],[77,89,99,88],[80,92,101,91],[86,96,105,97],[93,102,110,103],[98,107,113,106],[100,109,114,108],[104,111,116,112],[115,118,119,117],[0,2,6,13,8,3],[1,5,11,17,9,4],[7,14,22,34,24,15],[10,19,29,40,27,18],[12,21,32,43,30,20],[16,25,36,49,38,26],[23,35,47,53,41,28],[31,37,50,62,56,44],[33,46,58,70,57,45],[39,51,63,76,64,52],[42,54,66,79,67,55],[48,61,73,85,72,60],[59,71,83,89,77,65],[68,74,86,97,92,80],[69,81,93,103,94,82],[75,88,99,107,98,87],[78,91,101,109,100,90],[84,95,104,112,105,96],[102,108,114,118,115,110],[106,113,117,119,116,111],[0,3,7,15,23,28,19,10,5,1],[2,4,9,16,26,37,31,21,12,6],[8,13,20,30,42,55,46,33,22,14],[11,18,27,39,52,61,48,36,25,17],[24,34,45,57,69,82,71,59,47,35],[29,41,53,65,77,88,75,63,51,40],[32,44,56,68,80,91,78,66,54,43],[38,49,60,72,84,96,86,74,62,50],[58,67,79,90,100,108,102,93,81,70],[64,76,87,98,106,111,104,95,85,73],[83,94,103,110,115,117,113,107,99,89],[92,97,105,112,116,119,118,114,109,101]]};
       ...
       //make octahedron
       let oct1 = BABYLON.MeshBuilder.CreatePolyhedron('oct', {type: 1, size: 1}, this._scene);
       oct1.material = mat;
       oct1.position.x = -4;

       //make rhombathing
       let rhomb1 = BABYLON.MeshBuilder.CreatePolyhedron('rhomb', {type: 4, size: 1}, this._scene);
       rhomb1.material = mat;
       rhomb1.position.x = 4;

       //set up action manager for hover/click events

       fig.actionManager = new BABYLON.ActionManager(this._scene);
       oct1.actionManager = new BABYLON.ActionManager(this._scene);
       rhomb1.actionManager = new BABYLON.ActionManager(this._scene);

      fig.actionManager.registerAction(
        new BABYLON.SetValueAction(BABYLON.ActionManager.OnPointerOverTrigger, fig.material, 'emissiveColor', BABYLON.Color3.White())
      );

       oct1.actionManager.registerAction(
       );
       rhomb1.actionManager.registerAction(
